...
if (strlen($_SESSION['login']) == 0) {
    ...
    if (isset($_POST['submit'])) {
        try {
            ...
            $query = mysqli_prepare($bd, "INSERT INTO tblcomplaints (userId, category, subcategory, complaintType, state, noc, complaintDetails, complaintFile) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
            mysqli_stmt_bind_param($query, "isssssss", $uid, $category, $subcat, $complaintype, $state, $noc, $complaintdetails, $compfile);
            mysqli_stmt_execute($query);

            $complainno = mysqli_insert_id($bd);
          ...
          $mail->SMTPSecure = 'tls'; // Change this to 'ssl' if your server requires SSL
          $mail->Port = 587; // Change this to your SMTP port

          // Set email parameters
          $mail->addAddress($recipientEmail);
          $mail->Subject = 'New Complaint Registered';
          $mail->Body = 'A new complaint has been registered complainno '.$complainno;

          // Check if the email is sent successfully
          if ($mail->send()) {
              echo '<script>alert("Your complaint has been successfully filed, and an email has been sent.");</script>';
          } else {
...
<html lang="en">
  <head>
    ...
    <title>Student Support System | User Register Complaint</title>
